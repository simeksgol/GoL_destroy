...
{
	return upper_word | (mid_word >> 1) | (mid_word << 1) | lower_word;
}
...
static __force_inline void GoLGrid_flip_diagonally (const GoLGrid *src_gg, GoLGrid *dst_gg)
{
	...
	if (!src_gg || !src_gg->grid || src_gg->grid_rect.width != src_gg->grid_rect.height || src_gg->pop_x_on != 0 || src_gg->pop_y_on != 0 ||
			!dst_gg || !dst_gg->grid || dst_gg->grid_rect.width != src_gg->grid_rect.width || dst_gg->grid_rect.height != src_gg->grid_rect.height)
		return (void) ffsc (__func__);
